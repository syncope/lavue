...
#					delta=sum(np.multiply(tmp[i+3:i+7],np.array([1,256,65536,16777216],dtype='int64')))
					delta=(np.multiply(tmp[i+3:i+7],np.array([1,256,65536,16777216],dtype='int64'))).sum()
					if (delta > 2147483648):
						delta-=4294967296
					tmp[i]=delta
...
def eval_pildata(tmp):
	...
	boc=np.array([120, 45, 67, 66, 70, 95, 66, 89, 84, 69, 95, 79, 70, 70, 83, 69, 84],dtype='uint8')
	...
	dset_num_ele=np.array([88, 45, 66, 105, 110, 97, 114, 121, 45, 78, 117, 109, 98, 101, 114, 45, 111, 102, 45, 69, 108, 101, 109, 101, 110, 116, 115, 58],dtype='uint8')
	dset_fast_dim=np.array([88, 45, 66, 105, 110, 97, 114, 121, 45, 83, 105, 122, 101, 45, 70, 97, 115, 116, 101, 115, 116, 45, 68, 105, 109, 101, 110, 115, 105, 111, 110, 58],dtype='uint8')
	dset_sec_dim=np.array([88, 45, 66, 105, 110, 97, 114, 121, 45, 83, 105, 122, 101, 45, 83, 101, 99, 111, 110, 100, 45, 68, 105, 109, 101, 110, 115, 105, 111, 110, 58],dtype='uint8')
	dset_pad=np.array([88, 45, 66, 105, 110, 97, 114, 121, 45, 83, 105, 122, 101, 45, 80, 97, 100, 100, 105, 110, 103, 58],dtype='uint8')

	# search for data stream start
	if (flag == 0):
		try:
			idstart=tmp.tostring().index(inpoint.tostring())//tmp.itemsize
			idstart+=inpoint.size
		except:
			flag=1

		try:
			idstop=tmp.tostring().index(outpoint.tostring())//tmp.itemsize
		...
		spos[4]=idstart
		try:
			spos[0]=tmp.tostring().index(dset_num_ele.tostring())//tmp.itemsize
			spos[1]=tmp.tostring().index(dset_fast_dim.tostring())//tmp.itemsize
			spos[2]=tmp.tostring().index(dset_sec_dim.tostring())//tmp.itemsize
			spos[3]=tmp.tostring().index(dset_pad.tostring())//tmp.itemsize
		except:
			flag=1
		...
		if (flag == 0):
			image=0
			image=decompress_cbf_c(tmp[idstart:idstop+1], vals)
		else:
			image=np.array([0])
	return image
